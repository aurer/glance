...
export function Autocomplete ({placeholder, onChange, value}: AutocompleteProps) {
	...
	useEffect(() => {
		if (selectedCoin) {
			const activeElement = document.querySelector('[data-slot="autocomplete-list"] [aria-selected]');
			if (activeElement) {
	...
	return (
		<Popover open={popoverOpen} onOpenChange={setPopoverOpen} defaultOpen={true}>
			...
			<PopoverContent className="w-50 p-0 pointer-events-auto" align="start" onWheel={(e) => e.stopPropagation()}>
				...
				<div>
					{isLoading ? (
						...
						<ul data-slot="autocomplete-list" className="p-1 max-h-[300px] overflow-auto pointer-events-auto">
							{coins.map((coin) => (
								<li 
									key={coin.value}
...
function searchCoins(query: string): Promise<CoinListItem[]> {
	return axios.get(`https://api.coingecko.com/api/v3/search`, {
		params: { query },
	}).then((res) => {
		const coins = res.data.coins.map((coin: { id: string; name: string }) => ({
			label: coin.name,
			value: coin.id,
		}));
		return coins;
